...
3. Panel nastavení zobrazí všetky dostupné konfiguračné možnosti usporiadané podľa kategórií.
...
* **Typ**: Začiarkavacie políčko
* **Predvolené nastavenie**: Zapnuté (začiarknuté)
* **Popis**: Zapína automatickú kalibráciu odrazivosti pomocou detegovaných kalibračných cieľov vo vašich snímkach. Tým sa normalizujú hodnoty odrazivosti v celom súbore údajov a zabezpečia sa konzistentné merania bez ohľadu na podmienky osvetlenia.
* **Kedy vypnúť**: Vypnite len v prípade, ak chcete spracovávať surové, nekalibrované obrázky alebo ak používate iný kalibračný pracovný postup.
...
* Štandardná (rýchla, stredná kvalita)
  * Textúra (pomalá, najvyššia kvalita) \[Chloros+]
* **Predvolené nastavenie**: Štandardné (rýchle, stredná kvalita)
* **Popis**: Vyberá algoritmus demosaicingu, ktorý sa používa na konverziu surových údajov snímača Bayerovho vzoru na farebné obrázky. Metóda „Štandardné (rýchle, stredná kvalita)“ poskytuje optimálnu rovnováhu medzi rýchlosťou spracovania a kvalitou obrázkov. Metóda „S ohľadom na textúru (pomalá, najvyššia kvalita)“ \[Chloros+] používa vysokokvalitné odstránenie Bayerovho vzoru s ohľadom na okraje v kombinácii s modelom odšumovania AI/ML, ktorý odstraňuje takmer všetok šum odstránenia Bayerovho vzoru. Model s ohľadom na textúru vyžaduje na spustenie pamäť GPU (VRAM). Odporúčame ho používať, ak máte k dispozícii &gt;4 GB VRAM pre rýchlejšie spracovanie.
...
* **Kedy povoliť**: Odporúča sa vždy povoliť korekciu PPK, ak máte v súbore protokolu .daq údaje o spätnej väzbe expozície.
...
* **Popis**: Pri použití konfigurácie s dvoma kamerami priraďuje konkrétnu kameru k expozičnému pinu 2 na synchronizáciu času PPK.
* **Správanie automatického výberu**:
...
* **Popis**: Otvorí interaktívny panel, kde môžete vybrať a konfigurovať multispektrálne vegetačné indexy (NDVI, NDRE, EVI atď.) na výpočet počas spracovania obrazu. Môžete pridať viacero indexov, každý s vlastnými nastaveniami vizualizácie.
...
* **Odporúčania týkajúce sa formátu**:
  ...
  * **PNG (8-bit)**: Vhodný pre prezeranie na webe a všeobecnú vizualizáciu. Menšie veľkosti súborov s bezstratovou kompresiou, ale znížený dynamický rozsah.
...
* **Prípady použitia**:
  * Vytvorte šablóny pre rôzne kamerové systémy (RGB, multispektrálne, NIR)
